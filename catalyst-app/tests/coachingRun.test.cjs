...
async function run(t, response) {
  const saved = []
  const events = []
  const exists = fs.existsSync
  t.mock.method(fs, 'existsSync', file => file === paths.DB_PATH || exists(file))
  t.mock.method(config, 'loadConfig', () => ({ ai: { provider: 'anthropic', model: 'claude-fable-5-1' } }))
  ...
  registerApiHandlers((name, handler) => handlers.set(name, handler), () => target,
    undefined, undefined, { read: async () => ({ anthropic: 'fixture' }), write: async () => {} })
